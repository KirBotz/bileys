...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('text' in message) {
        ...
        if (message.pollSelectable < 0 || message.pollSelectable > message.pollValues.length) {
            throw new boom_1.Boom(`maxSelect in polls should be between 0 and ${message.pollValues.length} or equal to the items length`, { statusCode: 400 });
        }
        mPoll.name = message.pollName;
        mPoll.selectableOptionsCount = message.pollSelectable;
        mPoll.options = message.pollValues.map(value => WAProto_1.proto.Message.PollCreationMessage.Option.fromObject({
            optionName: value,
        }));
